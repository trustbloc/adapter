...
callbackUrl : <urlFromIssuerProfile>/cb?state=<state>&token=<token>
token: unique ID generated bu the adapter. This ID will be used to fetch the VC from issuer.

### 1.4 Issuer VC API - HTTP POST <urlFromIssuerProfile>/credential
When RP asks for the user credential, the Issuer adapter calls this API to get the user credential from the issuer.
